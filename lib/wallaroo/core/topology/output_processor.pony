...
primitive OutputProcessor
  fun apply[D: Any val](next_runner: Runner, metric_name: String,
    pipeline_time_spent: U64, output: (D | Array[D] | None), key: Key,
    producer_id: RoutingId, producer: Producer ref, router: Router,
    target_id_router: TargetIdRouter, i_msg_uid: MsgId,
    frac_ids: FractionalMessageId, computation_end: U64, metrics_id: U16,
    ...
    | let os: Array[Out] val =>
      ...
      for (frac_id, o) in os.pairs() do
        ...
        (let f, let ts) = next_runner.run[Out](metric_name,
          ...
          target_id_router, i_msg_uid, o_frac_ids, computation_end,
